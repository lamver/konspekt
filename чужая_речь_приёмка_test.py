...
def main() -> int:
    ...
    try:
        ...
        # Кладём реплики, как будто распознавание уже отработало: чужой
        # ролик в системном звуке и своя речь. Ради этого всё и делалось —
        # именно они портят саммари, если оставит�� их молча.
        from app.core.models import Speaker, TranscriptSegment
        служба.store.add_segment(TranscriptSegment(
            meeting_id=mid, speaker=Speaker.THEM,
            text="озвучка чужого ролика", start=1.0, end=3.0,
        ))
        служба.store.add_segment(TranscriptSegment(
            meeting_id=mid, speaker=Speaker.ME,
            text="моя настоящая реплика", start=2.0, end=4.0,
        ))
        ...
        # Записанное до ответа должно быть помечено: тихо оставить его
        # нельзя, оно и есть источник испорченных заметок.
        проверить(ответ.get("помечено") == 1,
                  f"записанное до выключения помечено (помечено: {ответ.get('помечено')})")
        реплики = {с.text: с for с in служба.store.list_segments(mid)}
        проверить(реплики["озвучка чужого ролика"].doubtful,
                  "чужой ролик помечен как сомнительный")
        проверить(not реплики["моя настоящая реплика"].doubtful,
                  "своя речь не тронута")
        проверить("озвучка чужого ролика" not in служба.transcript_text(mid),
                  "в заметки чужой ролик уже не попадёт")
        проверить("моя настоящая реплика" in служба.transcript_text(mid),
                  "своя речь в заметки по-прежнему идёт")
        ...
        time.sleep(2.0)
        # Дорожка теперь есть всегда, но при выключенном звуке она
        # только слушает: это нужно для обратного случая (собеседник
        # заговорил в Zoom). Проверяем не наличие дорожки, а то, что
        # звук не попадает ни в файл, ни в расшифровку.
        трек = служба.capture._tracks.get(devices.TRACK_THEM)
        проверить(трек is None or трек._только_слушать,
                  "следующая встреча системный звук уже не пишет")
        if трек is not None:
            проверить(трек._writer is None,
                      "файл системного звука на второй встрече не заводится")
