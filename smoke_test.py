...
def main() -> int:
    ...
    # Ждём первое событие, а не спим наугад. Раньше здесь стоял сон в
    # 0.35с: на моей машине хватало, а на сервере сборки, где всё
    # медленнее, звук не успевал дойти, и проверка падала без всякой
    # поломки. Ждём до пяти секунд, но уходим сразу, как только пришло.
    предел = time.monotonic() + 5.0
    while not levels and time.monotonic() < предел:
        time.sleep(0.05)
