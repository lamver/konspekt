...
if checked < 3:
    ...
    raise SystemExit(0)
# Одна-две ошибки допустимы: VoxLingua не идеален, а текстовая сверка
# (py3langid) в любом случае исправит. Десятки ошибок (как было при
# пороге 0.5) — уже регресс, его ловит проверка на боевой встрече ниже.
assert wrong <= 2, f"{wrong} русских записей ушли бы не в ту модель"
if wrong:
    print(f"[ok] {wrong} запись определена неверно, но текстовая сверка исправит")
else:
