...
def _ответ_из_уведомления() -> str | None:
    ...
    """
    известные = {"off", "keep", "on", "rec", "skip"}
        ...
        while True:
            ...
            try:
                if ответ == "off":
                    ...
                    # Сторож заметил разговор при выключенной записи, и
                    # человек согласился её начать прямо из угла экрана.
                    итог = service.записать_замеченный_разговор()
                    log.info("Запись начата кнопкой в уведомлении: %s",
                             "да" if итог.get("ok") else "не вышло")
                elif ответ == "skip":
                    service.не_записывать_замеченный_разговор()
                    log.info("Человек отказался записывать замеченный разговор")
            except Exception:
...
def main() -> int:
    ...
    tray = TrayIcon(
        on_new_meeting=service.create_meeting,
        # Галочку «показывать уведомления» человек может снять в любой
        # момент, поэтому спрашиваем настройку каждый раз, а не
        # запоминаем значение при запуске.
        разрешены=lambda: service.settings.внимание.уведомления,
    )
    ...
    def on_started() -> None:
        ...
        # Сторож слушает системный звук, пока запись не идёт: человек
        # мог открыть Zoom и забыть нажать кнопку. Поднимаем только
        # теперь, когда окно и трей готовы показать вопрос.
