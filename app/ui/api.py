...
class Api:
    ...
    def turn_on_system_audio(self) -> dict[str, Any]:
        """Ответ «это собеседник, пишите»: включить системный звук на ходу.

        Обратный случай: встреча шла без системного звука, а собеседник
        заговорил в Zoom.
        """
    ...
    # --- диктовка в поле ввода самого окна --------------------------------
    #
    # Вопрос к встрече быстрее сказать, чем напечатать, особенно длинный.
    # От обычной диктовки отличается тем, что текст никуда не печатается:
    # он возвращается сюда строкой, и окно кладёт его в поле, где человек
    # успеет поправить сказанное перед отправкой (задача №26).

    def start_field_dictation(self) -> dict[str, Any]:
        """Начать слушать для поля ввода."""
        return self._service.начать_диктовку_в_окно()

    def finish_field_dictation(self) -> dict[str, Any]:
        """Остановить и вернуть распознанный текст."""
        return self._service.закончить_диктовку_в_окно()

    def cancel_field_dictation(self) -> dict[str, Any]:
        """Бросить запись: человек передумал."""
        return self._service.отменить_диктовку_в_окно()
