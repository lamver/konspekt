...
# Имя, в котором человеку не за что зацепиться. Держим правило узким:
# ошибиться в эту сторону дешевле. Не узнали идентификатор — человек
# увидит некрасивое имя, как раньше. А вот переименовать «Планерка 7
# мая» в «Запись 14.07» значит отнять то, что он написал сам.
_ИДЕНТИФИКАТОР = re.compile(
    r"""^(
        [0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}  # uuid
        | [0-9a-f]{16,}                    # длинная шестнадцатеричная каша
        | audio | recording | record | sound | voice | new\ recording
        | \d{8,}                           # 20260714165030
    )
...
def похоже_на_идентификатор(имя: str) -> bool:
    """Имя файла, которое ничего не говорит человеку."""
    return bool(_ИДЕНТИФИКАТОР.match(имя.strip()))


def время_файла(path: Path) -> str:
    """Дата и время записи: «14.07 16:50».

    Берём время создания, а если система его не хранит — время правки.
    Точность тут не главное: человеку нужно зацепиться за «когда», а не
    установить хронологию до секунды.
    """
    try:
        отметка = path.stat()
        когда = getattr(отметка, "st_birthtime", None) or отметка.st_mtime
    except OSError:
        когда = time.time()
    return time.strftime("%d.%m %H:%M", time.localtime(когда))


def _примета(имя: str) -> str:
    """Короткий хвост, чтобы две записи одной минуты не слились.

    Для «audio (1)» это номер копии, для длинного идентификатора —
    первые знаки. Человеку эта примета ничего не говорит, но и не
    должна: её работа в том, чтобы строки в списке различались.
    Пустую строку возвращаем, когда цепляться не за что.
    """
    имя = имя.strip()
    номер = re.search(r"\((\d+)\)\s*$", имя)
    if номер:
        return f" ({номер.group(1)})"
    # Слово вроде «audio» или «recording» в примету не годится: «Запись
    # 14.07 16:50 (audi)» выглядит как опечатка и ничего не добавляет.
    # А вот кусок идентификатора различает записи одной минуты.
    только_знаки = re.sub(r"[^0-9a-zA-Zа-яА-Я]", "", имя)
    if len(только_знаки) >= 12:
        return f" ({только_знаки[:4]})"
    return ""
...
class AudioInfo:
    ...
    @property
    def title(self) -> str:
        """Заголовок встречи по файлу.

        Обычно берём имя файла: «Планерка 7 мая» лучше любой выдумки.
        Но записи часто приходят из Zoom, диктофона или мессенджера, и
        зовутся `7246a5e7-0c69-4c6d-a5de-560b15b42cd8` или `audio (1)`.
        Такое имя человеку не говорит ничего: в живой базе таких записей
        оказалось тридцать из девяноста восьми, и найти среди них нужную
        нельзя даже глазами.

        Для таких берём дату и время самого файла: «Запись 14.07 16:50»
        не говорит о содержании, но хотя бы отвечает на вопрос «когда»,
        а по времени человек вспоминает разговор.

        Если и время совпало (скачали пачку разом, и у всех одна
        минута), дописываем хвост исходного имени. Две неразличимые
        строки в списке — та же беда, от которой уходим.
        """
        имя = (self.path.stem or self.path.name).strip()
        if имя and not похоже_на_идентификатор(имя):
            return имя
        когда = время_файла(self.path)
        примета = _примета(имя)
        return f"Запись {когда}{примета}"
