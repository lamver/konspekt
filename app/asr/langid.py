...
# Порог поднят с 0.5 после разбора боевой встречи (issue #2): на короткой
# русской речи (в среднем 1.2с на фразу) модель уверенно ошибается сама
# на себя. Прогон 170 настоящих реплик чисто русской встречи через
# детектор без ограничений роутера дал 27 ложных «иностранных» ответов
# при пороге 0.5, из-за чего половина фраз улетала в Whisper — тот на
# короткой фразе тратит около 11с против 0.15-0.25с у GigaAM — и текст
# превращался в кашу («и в транскрипт» → чешский «și le transcript.»).
# При пороге 0.85 ложных ответов осталось 1 из 170. Выше, 0.9, точность
# та же, но уже режет пограничные настоящие случаи (английская тестовая
# фраза шла с уверенностью 0.858), так что 0.9 не берём: это была бы
# защита ради красивой цифры, а не ради дела.
MIN_CONFIDENCE = 0.85
