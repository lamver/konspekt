...
MAX_SEGMENT = 8.0             # принудительная резка, чтобы не копить вечно, с
PAD = 0.15                    # прихватываем немного до и после, с
QUIET_SEARCH = 2.0            # где искать тихий стык при резке длинной речи, с
# Про MAX_SEGMENT. Было 18 секунд, и на речи без пауз (подкаст, лекция,
# монолог) это означало, что готовый текст ждёт до 16 секунд: фраза
# копится, пока не упрётся в предел, и только тогда уходит в модель.
# Черновики скрашивают ожидание, но готовая реплика всё равно приходит
# с опозданием, и человек видит, как программа отстаёт от речи.
# На боевой записи подкаста: при 18с худшее ожидание 16.2с, при 8с —
# 8.6с. Текст при этом не страдает: разрез ищется по самому тихому
# месту в окне QUIET_SEARCH, и на трёх боевых записях ни один из 67
# кусков не оборвался посреди звука ни при 18с, ни при 8с.
# Меньше 8 не берём: слишком короткие куски лишают модель контекста,
# а GigaAM всё равно добивает вход тишиной до 6 секунд.
