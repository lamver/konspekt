...
MAX_PENDING = 48
# Сколько мест в очереди досчёт не занимает никогда.
#
# Досчёт кладёт куски с block=True: ждёт места сколько нужно, торопиться
# ему некуда. Но человек может начать новую встречу, не дожидаясь конца
# досчёта, и тогда они делят одну очередь. Живая речь ждать не может —
# за ней стоит захват звука, и место она не ждёт, а теряет. Забитая
# досчётом очередь означала, что живая речь новой встречи пропадала
# целиком. Поэтому досчёт останавливается, не доходя до потолка, и
# оставляет запас тому, что происходит прямо сейчас.
BACKFILL_RESERVE = 16
...
class TranscriptionQueue:
    ...
    def __init__(
        ...
        # Программа закрывается. Досчёт ждёт места в очереди в своём
        # потоке, и без этого признака он ждал бы его до конца света,
        # не давая приложению закрыться.
    ...
    def _put(self, job: Job, block: bool = False) -> None:
        if block:
            # Досчёт и разбор файла: ждём места сколько надо. Терять речь
            # тут нельзя, файл никуда не убежит, в отличие от живого звука.
            #
            # Но занимать всю очередь при этом нельзя тоже: рядом может
            # идти живая запись, а она ждать не умеет и просто теряет
            # куски. Поэтому останавливаемся, не доходя до потолка, и
            # держим запас под то, что говорится прямо сейчас.
            предел = max(1, MAX_PENDING - BACKFILL_RESERVE)
            while not self._stopping:
                if self._queue.qsize() < предел:
                    try:
                        self._queue.put(job, timeout=0.2)
                        return
                    except queue.Full:
                        continue
                # Место занято живой речью: ждём, пока она пройдёт вперёд.
                time.sleep(0.05)
            # Досюда доходим только при закрытии программы. Кусок в
            # очередь не попал, поэтому запоминаем его как пропущенный:
            # звук лежит в записи, и следующий досчёт возьмёт его оттуда.
            # Молча выбросить было бы возвратом к той самой дыре в
            # расшифровке, ради которой досчёт и написан.
            if not job.draft:
    ...
    def stop(self, timeout: float = 30.0) -> None:
        ...
        if thread is None:
            return
        # Отпускаем досчёт, если он сейчас ждёт места в очереди: иначе
        # он держал бы поток и мешал закрытию программы.
        self._stopping = True
