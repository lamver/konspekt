...
class TrayIcon:
    def __init__(self, on_new_meeting=None, разрешены=None) -> None:
        self._on_new_meeting = on_new_meeting
        # Показывать ли системные уведомления. Функцией, а не значением:
        # человек может снять галочку в настройках посреди работы, и
        # спрашивать надо каждый раз заново.
        self._разрешены = разрешены or (lambda: True)
        ...
        # Разговор при выключенной записи. Тут уведомление вообще
        # единственный способ докричаться: окно свёрнуто, а человек
        # уверен, что встреча пишется.
        bus.on(SYSTEM_SPEECH_NOTICED, self._спросить_про_запись)

    def _спросить_про_запись(self, payload: dict) -> None:
        """Сторож услышал разговор, а запись не идёт: предложить начать."""
        секунд = payload.get("секунд") or 30
        self._показать(
            текст=(f"На компьютере уже {секунд} секунд идёт разговор, "
                   "а запись не включена."),
            кнопка1="Записать", ответ1="rec",
            кнопка2="Не нужно", ответ2="skip",
        )

    def _показать(self, текст: str, кнопка1: str, ответ1: str,
                  кнопка2: str, ответ2: str) -> None:
        """Тост с двумя кнопками, а при неудаче — облачко у трея.

        Сначала пробуем тост Windows: он умеет кнопки, и ответить можно
        не открывая окно. Если не вышло (не та система, отключены
        уведомления), падаем на обычное «облачко»: оно без кнопок, но
        хотя бы предупредит.
        """
        if not self._разрешены():
            # Человек снял галочку «показывать уведомления»: плашка в
            # окне всё равно покажется, а лезть поверх чужой работы он
            # запретил.
            return
    ...
    def _спросить_про_звук(self, payload: dict) -> None:
        ...
        self._показать(текст, кнопка1, ответ1, кнопка2, ответ2)
