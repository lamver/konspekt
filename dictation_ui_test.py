...
def _сцена(w, service, итог: dict) -> None:
    ...
    #
    # Ждём щедро: на своей машине окно поднимается за пару секунд, а на
    # холодном сборочном агенте WebView2 разворачивается первый раз в
    # жизни и не укладывается и в полминуты. Ровно на этом проверка и
    # краснела в CI, оставаясь зелёной локально.
    #
    # Не дождавшись, падаем словами про незагруженную страницу. Прежде
    # тут молча шли дальше, и первый же вызов кода страницы выдавал
    # «showPrefsTab is not defined» — сообщение, по которому думаешь на
    # пропавшую функцию, а не на неоткрытое окно.
    for _ in range(240):
        try:
            if w.evaluate_js(
        ...
        try:
            состояние = w.evaluate_js(
                "document.readyState + ', app.js '"
                " + (typeof loadDictation === 'function' ? 'загружен' : 'нет')"
                " + ', словарь '"
                " + (window.__konspekt_i18n_ready === true ? 'готов' : 'нет')"
            )
        except Exception:
            pass
        raise AssertionError(f"страница не поднялась за 120 с (состояние: {состояние})")
...
def main() -> int:
    ...
    def сторож() -> None:
        ...
        Срок заведомо больше, чем ждёт сама сцена: иначе на медленной
        машине сторож убьёт окно ровно в тот момент, когда WebView2
        почти поднялся, и вместо внятной жалобы мы получим невнятную.
        """
        for _ in range(240):
            time.sleep(1)
