...
def main() -> int:
    ...
    def сторож() -> None:
        ...
        Срок с запасом над ожиданием страницы: сторож не должен
        срабатывать раньше, чем сцена успеет пожаловаться сама.
        """
        for _ in range(240):
            time.sleep(1)
            if "готово" in итог or "ошибка" in итог:
                return
        итог.setdefault("ошибка", "сцена не успела за 240с")
        try:
            окно.quit()
        except Exception:
            pass
    ...
    if "ошибка" in итог:
        ...
        проверить(bool(итог.get("досчёт_не_перебит")),
                  f"загрузка модели не перебивает строку досчёта "
                  f"({str(итог.get('текст_после_модели'))!r})")
...
def _сцена(w, service, bus, RECOGNITION_BACKFILL,
    ...
    #
    # Щедро: на холодном сборочном агенте WebView2 разворачивается
    # первый раз в жизни и десяти секунд ему мало. Дома хватает одной,
    # поэтому большой потолок ничего не замедляет — выход по готовности.
    for _ in range(1200):
        time.sleep(0.1)
        try:
    ...
    # Загрузка модели не имеет права перебить строку досчёта. Место под
    # сообщение одно, а досчёт идёт молча минутами: перебив его, мы
    # оставим человека думать, что расшифровка уже полная, и он отправит
    # в заметки половину встречи. Раньше это падало через раз, в
    # зависимости от того, успела ли соседняя проверка дёрнуть загрузку
    # модели, и выглядело как случайность.
    w.evaluate_js("window.__konspekt_event({topic: 'model.download',"
                  " state: 'downloading', bytes: 1, total: 100})")
    итог["досчёт_не_перебит"] = w.evaluate_js(
        "!!(ui.toastText && ui.toastText.textContent.indexOf('Досчитываем') >= 0)")
    итог["текст_после_модели"] = w.evaluate_js(
        "ui.toastText ? ui.toastText.textContent : ''")
