...
try:
    ...
    if боевая.exists():
        ...
        # На сервере сборки боевой базы нет: заводим свою, чтобы было
        # что спасать. Раньше эта ветка вызывала методы, которых у Store
        # никогда не было, и падала — но узналось об этом только когда
        # проверки перестали трогать живой профиль и ветка наконец
        # заработала. Немой код, который никогда не исполнялся.
        from app.core.models import Meeting, Speaker, TranscriptSegment

        склад = Store(str(копия))
        встреча = склад.create_meeting(Meeting(title="Проверка отката"))
        склад.add_segment(TranscriptSegment(
