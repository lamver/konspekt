...
def check_latest_format_guard() -> None:
    ...
    установленные копии. До 0.9.0 они ждут строку и покажут человеку
    ...
    # Переводы для версии, которая их не понимает, — отказ.
    try:
        обновить.собрать_новость(Аргументы(ru="а", en="b"), "0.8.0")
    except SystemExit as e:
        assert "0.9.0" in str(e), "в отказе не сказано, с какой версии можно"
    else:
        raise AssertionError(
    ...
    # Начиная с версии, которая понимает словарь, согласие не нужно.
    принято = обновить.собрать_новость(Аргументы(ru="а", en="b"), "0.9.0")
    assert принято == {"ru": "а", "en": "b"}, "переводы отвергнуты там, где безопасны"
