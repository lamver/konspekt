...
# Кириллические соседи русского уходят в GigaAM — и метку получают
# русскую, а не ту, что послышалась акустике.
#
# VoxLingua постоянно принимает русскую речь за украинскую или
# белорусскую: на боевой базе так помечено 596 реплик из 4294, и у 592
# из них текст оказался русским. Отправить их всё равно некуда, кроме
# GigaAM, а он не умеет ничего, кроме русского. То есть текст в любом
# случае русский, и ярлык «УК» на нём — просто вранье человеку: он
# видит в расшифровке украинский, которого там нет.
class _UkDetector:
    def detect(self, pcm, sample_rate=16000):
        return ("uk", 0.9)
...
if segs:
    assert segs[0].lang == "ru", (
        f"реплику распознал GigaAM, то есть текст русский, а помечена она "
        f"как {segs[0].lang!r}: человек увидит в расшифровке язык, "
        f"которого там нет"
    )
    text = segs[0].text
    assert any("а" <= c.lower() <= "я" for c in text), "кириллическая речь ушла не в GigaAM"
    print("[ok] услышанный украинский остался в GigaAM и помечен русским")
