...
window.__konspekt_event = function (payload) {
  switch (payload.topic) {
    ...
    case 'recording.silent':
      ...
      showToast(silentMessage(payload), 10000);
      break;
...
function onModelProgress(payload) {
  if (payload.state === 'downloading') {
    ...
    //
    // Но не поверх досчёта. Место под сообщение одно, а досчёт идёт
    // молча минутами: перебив его, мы оставим человека думать, что
    // расшифровка уже полная, и он отправит в заметки половину встречи.
    // Загрузка модели при этом видна отдельной плашкой с процентами.
    if (!state.modelToastShown && !state.backfilling) {
...
/**
 * Текст предупреждения о немой дорожке — на языке человека.
 *
 * Собираем в окне, а не в Python: интерфейс бывает на четырёх языках, и
 * русская фраза посреди английского — та же беда, что ключ перевода
 * вместо текста. Готовый русский текст из события остаётся запасным
 * вариантом на случай, когда словарь ещё не доехал через мост: он
 * честнее ключа.
 *
 * И главное — говорим, что делать. Предупреждение «за 10 секунд не было
 * ни звука» само по себе только пугает: человек видит, что встреча
 * пишется и реплики идут, а внизу тревога без выхода (задача №27).
 */
function silentMessage(payload) {
  const данные = payload || {};
  const дорожка = данные.track === 'me' ? 'recording.silent_mic'
    : 'recording.silent_system';
  const куда = tЕслиЕсть(дорожка);
  if (!куда) return данные.message || t('recording.silent');
  const секунд = Number(данные['секунд']) || 0;
  const начало = tЕслиЕсть('recording.silent_where', { track: куда, seconds: секунд });
  const совет = tЕслиЕсть('recording.silent_hint');
  return [начало, совет].filter(Boolean).join(' ')
    || данные.message || t('recording.silent');
}
